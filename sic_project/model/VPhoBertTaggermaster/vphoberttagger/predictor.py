from model.VPhoBertTaggermaster.vphoberttagger.arguments import get_predict_argument
from model.VPhoBertTaggermaster.vphoberttagger.constant import LABEL_MAPPING, MODEL_MAPPING
...
class ViTagger(object):
    def __init__(self, model_path: Union[str, os.PathLike],  no_cuda=False):
        ...
        jar_path = "/opt/airflow/sic_project/model/VPhoBertTaggermaster/vncorenlp/VnCoreNLP-1.1.1.jar"
    ...
    @staticmethod
    def load_model(model_path: Union[str, os.PathLike],  device='cpu'):
        if device == 'cpu':
...
# ✅ Di chuyển phần khởi tạo predictor ra ngoài extract()
predictor_instance = None  # Khởi tạo toàn cục

def extract(text):
    global predictor_instance
    if predictor_instance is None:
        # 👉 Chỉ nên gọi get_predict_argument() khi bạn chủ động cung cấp argument
        # Vì trong Airflow hoặc khi import, bạn nên set sẵn model_path
        from argparse import Namespace
        args = Namespace(model_path="model_path/your_model.pt", no_cuda=True)  # <-- Gán cứng hoặc lấy từ biến môi trường
        predictor_instance = ViTagger(args.model_path, no_cuda=args.no_cuda)
    
    return predictor_instance(text)
